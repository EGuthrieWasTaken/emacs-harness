...
BIN_DIR = Path(os.environ.get("EH_BIN_DIR", str(EH_ROOT / "bin")))
...
# unix(7): struct sockaddr_un's sun_path is 108 bytes on Linux, and bind(2)
# needs room in there for the trailing NUL, so 107 usable bytes -- past that,
# `server-start' fails inside Emacs with nothing but a silent, empty
# emacs.log, which is why this is checked up front instead of surfacing as
# "emacs server socket never appeared" further down.
...
def _check_socket_path_length(socket_path: Path) -> None:
    n = len(os.fsencode(str(socket_path)))
    if n > UNIX_SOCKET_PATH_LIMIT:
        raise EhError(
            f"session socket path is {n} bytes, over the "
            f"{UNIX_SOCKET_PATH_LIMIT}-byte unix domain socket path limit: {socket_path} "
            f"-- point EH_VAR_RUN at a shorter directory",
            EXIT_USAGE,
        )
...
(setq eh-elisp-dir "{ELISP_DIR}")
(setq eh-bin-dir "{BIN_DIR}")
