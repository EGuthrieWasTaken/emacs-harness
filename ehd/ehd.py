...
def _translate_key(desc: str) -> str:
    """Best-effort Emacs key description -> xdotool key syntax, per DESIGN §6.3.
...
def host_doctor_checks() -> list[dict]:
    checks = []
    for tool in ("Xvfb", "openbox", "emacsclient", "xdotool", "convert", "ffmpeg"):
...
async def handle(mgr: SessionManager, req: dict) -> dict:
    ...
    if cmd == "scroll":
        if args.get("lines"):
            ...
            await emacs_eval(sess, f'(pixel-scroll-precision-scroll-up {int(args["pixels"])})', timeout)
        return dict(ok=True, exit_code=EXIT_OK)
...
async def handle_diff_shot(sess: Session, args: dict, timeout: float) -> dict:
    ...
    baseline = PROFILES_ROOT / sess.profile / "baselines" / "current" / sess.geometry / f"{name}.png"
    actual_path = sess.run_dir / f"{name}.actual.png"
    diff_path = sess.run_dir / f"{name}.diff.png"
    await emacs_eval(sess, f'(eh-shot-to-file-json {_elisp_str(str(actual_path))})', timeout)
    if not baseline.exists():
        return dict(ok=False, error={"message": f"no baseline for {name}; run `eh baseline accept {name}`"},
                    actual=str(actual_path), exit_code=EXIT_FAIL)
    proc = subprocess.run(
        ["compare", "-metric", "AE", "-fuzz", "1%", str(actual_path), str(baseline), str(diff_path)],
        capture_output=True, text=True, timeout=timeout,
    )
    try:
        changed = float(proc.stderr.strip() or proc.stdout.strip() or "0")
    except ValueError:
        changed = float("inf")
    total = _png_pixel_count(actual_path)
    ratio = (changed / total) if total else 1.0
    ok = ratio <= tolerance
    return dict(ok=ok, changed_pixels=changed, total_pixels=total, ratio=ratio,
                actual=str(actual_path), diff=str(diff_path), baseline=str(baseline),
...
async def handle_baseline_accept(sess: Session, args: dict) -> dict:
    name = args.get("name")
    baseline_dir = PROFILES_ROOT / sess.profile / "baselines" / "current" / sess.geometry
    baseline_dir.mkdir(parents=True, exist_ok=True)
    names = [name] if name else [p.stem.replace(".actual", "")
                                  for p in sess.run_dir.glob("*.actual.png")]
    accepted = []
    for n in names:
        src = sess.run_dir / f"{n}.actual.png"
        if src.exists():
            shutil.copy(src, baseline_dir / f"{n}.png")
            accepted.append(n)
    return dict(ok=True, accepted=accepted, exit_code=EXIT_OK)
...
async def handle_run(mgr: SessionManager, session_name: Optional[str], args: dict,
    ...
    if reuse and session_name in mgr.sessions and not mgr.sessions[session_name].dead:
        ...
        ephemeral_name = session_name or f"run-{profile}-{uuid.uuid4().hex[:8]}"
